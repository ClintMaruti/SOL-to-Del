...
} from "./itinerariesList.utils";
import type { ItinerariesFilterChip, ItinerariesFilterChipKey } from "./types";

export function useVisibleItinerariesFilterChips(
  ...
  const chipMeasureRefs = useRef(
    new Map<ItinerariesFilterChipKey, HTMLDivElement | null>()
  );

  const setChipMeasureRef = useCallback(
    (key: ItinerariesFilterChipKey) => (node: HTMLDivElement | null) => {
      if (node) {
  ...
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- chip overflow measurement (same pattern as margin-rules list)
