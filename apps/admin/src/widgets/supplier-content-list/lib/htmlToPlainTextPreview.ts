/**
 * Plain text for the supplier content list "Text" column (widget-local).
...
export function htmlToPlainTextPreview(html: string): string {
  ...
  try {
    const doc = new DOMParser().parseFromString(
      `<div data-supplier-content-preview-root="1">${normalized}</div>`,
      "text/html"
    );
    const root = doc.querySelector("[data-supplier-content-preview-root]");
